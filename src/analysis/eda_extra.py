"""
Analisis visual complementario al EDA principal.
Genera:
  1. Heatmap de correlacion OHLCV
  2. Grafico ACF/PACF de log_return
  3. Scatter rango diario (High-Low) vs Close
"""
...
def plot_correlation_heatmap(df: pd.DataFrame) -> str:
    """Heatmap de correlacion entre OHLCV."""
    ...
    ax.set_title("Matriz de Correlacion - OHLCV (2015-2026)")
    fig.tight_layout()
...
def plot_acf_pacf(df: pd.DataFrame) -> str:
    """ACF y PACF de log_return para evaluar estructura temporal."""
    lr = np.log(df["Close"]).diff().dropna()
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))
    plot_acf(lr, lags=60, alpha=0.05, ax=ax1, title="ACF - log_return (60 lags)")
    plot_pacf(lr, lags=60, alpha=0.05, ax=ax2, title="PACF - log_return (60 lags)", method="ywm")
    fig.tight_layout()
    path = os.path.join(FIG_DIR, "eda_acf_pacf.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Figura guardada: {path}")
    return path
...
def plot_range_vs_close(df: pd.DataFrame) -> str:
    """Scatter: rango diario (High-Low) vs Close, coloreado por Volume."""
    df_plot = df.copy()
    ...
    scatter = ax.scatter(df_plot["daily_range"], df_plot["Close"],
                         c=df_plot["log_vol"], cmap="viridis",
                         alpha=0.6, s=15)
    cbar = fig.colorbar(scatter, ax=ax, label="log(Volume)")
    ...
    ax.plot(x, b + m * x, color="red", linewidth=1, linestyle="--",
            label=f"Tendencia lineal (r={df_plot['daily_range'].corr(df_plot['Close']):.2f})")
    ax.legend()

    fig.tight_layout()
    path = os.path.join(FIG_DIR, "eda_range_vs_close.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Figura guardada: {path}")
    return path
